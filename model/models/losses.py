...
from model.utils.math import (log_density_gaussian, log_importance_weight_matrix,
...
def kl_standard_normal(q_z):
    mu = q_z.mean
    var = q_z.variance
    latent_kl = 0.5 * (mu ** 2 + var - 1 * torch.log(var) - 1).mean(dim=0)
    total_kl = latent_kl.mean()
    return total_kl

def kl_normal_normal(q_z, p_z):
    return torch.mean(kl_divergence(q_z, p_z))
...
def compute_mmd(sample_qz, sample_pz, kernel='im'):
    if kernel == 'g':
        # use gaussian kernel
        x_kernel = gaussian_kernel(sample_qz, sample_qz)
        y_kernel = gaussian_kernel(sample_pz, sample_pz)
        xy_kernel = gaussian_kernel(sample_qz, sample_pz)
        mmd = x_kernel.mean() + y_kernel.mean() - 2*xy_kernel.mean()
    elif kernel == 'im':
        # use im kernel
        mmd = im_kernel(sample_qz, sample_pz)
    else:
        raise NotImplementedError
    return mmd

def im_kernel(sample_qz, sample_pz, z_var=1):
