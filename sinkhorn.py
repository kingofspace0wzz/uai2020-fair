...
def sinkhorn_loss_primal(x,y,epsilon,n,niter) :
	...
	mu = 1./n*torch.ones(n).to(x.device) 
	nu = 1./n*torch.ones(n).to(x.device)
...
def test():
    X = Normal(torch.zeros(3,3), torch.ones(3,3))
    Y = Normal(torch.randn(3,3), torch.ones(3,3)*2)
    x = X.sample()
    y = Y.sample()
    cost = sinkhorn_loss_primal(x, y, 1, 3, 5)
    print(cost)
