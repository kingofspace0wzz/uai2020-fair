...
class SimpleClassifier(nn.Module):
    def __init__(self, input_dim, out_dim, code_dim, hidden_dim=64):
        super().__init__()
        ...
        self.z_to_y = nn.Linear(code_dim, out_dim)
...
def train(model):
    ...
    try:
        for epoch in range(args.epochs):
            ...
            for i, data in enumerate(tqdm(train_iter)):
                ...
                if args.ite and epoch >= args.wstart:
                    for _ in range(args.critic_iter):
                        ...
                        z_0 = model.encoder(x_0) # z ~ q(z|do(s), x)
                        dis_int = critic(z_0)
            ...
            print('-'*90)
            print('Epoch: {:3d} | wloss {:5.4f} | acc {:5.2f}'.format(epoch, w_loss, acc))
...
def evaluate(model):
    ...
    print('-'*90)
    print('Test | wloss {:5.2f} | acc {:5.2f} | ite {:5.4f}'.format(w_loss, acc, it_estimate))
    with open(args.log, 'a') as fd:
        print('Test | wloss {:5.2f} | acc {:5.2f} | ite {:5.4f}'.format(w_loss, acc, it_estimate), file=fd)
...
if __name__ == "__main__":
    ...
    for _, (batch, _, _) in enumerate(train_iter):
        ...
        break
    model = SimpleClassifier(input_dim+1, 2, args.latent_size, args.hidden_dim).to(device)
